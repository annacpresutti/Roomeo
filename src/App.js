...
const membersSeed = [
  { id: "anna", name: "Anna", email: "acp5645@psu.edu", color: "#8B5CF6" },
  { id: "xiaowen", name: "Xiaowen", email: "xpj5068@psu.edu", color: "#22C55E" },
  { id: "jake", name: "Jake", email: "jrc6728@psu.edu", color: "#F59E0B" },
  { id: "tenley", name: "Tenley", email: "tjl6106@psu.edu", color: "#38BDF8" }
];
...
function DollarIcon({ active }) { return (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" className={active ? "stroke-white" : "stroke-zinc-400"}><path d="M12 2v20" strokeWidth="1.6"/><path d="M16 6c0-2-2-3.5-4-3.5S8 4 8 6s2 3 4 3 4 1 4 3-2 3.5-4 3.5S8 14 8 12" strokeWidth="1.6"/></svg>); }
...
function ChoresScreen({ members, chores, setChores }) {
  ...
  const order = ["anna", "tenley", "jake", "xiaowen"];

  const slices = useMemo(() => {
    const byId = Object.fromEntries(members.map(m => [m.id, m]));
    const counts = order.map(id => ({
      m: byId[id],
      count: chores.filter(c => c.assignee.id === id).length
    }));
    const total = counts.reduce((a, b) => a + b.count, 0);

    if (total === 0) {
      let acc = 0;
      const sweep = 360 / counts.length;
      return counts.map(({ m }) => {
        const start = acc, end = acc + sweep;
        acc = end;
        return { m, start, end, sweep, mid: (start + end) / 2, count: 0 };
      });
    }
    ...
    return counts
      ...
      .map(({ m, count }) => {
        ...
        const start = acc, end = acc + sweep;
        acc = end;
        return { m, start, end, sweep, mid: (start + end) / 2, count };
      });
  }, [members, chores]);
  ...
  const labelPoint = (cx, cy, outerR, midDeg, sweepDeg) => {
    if (sweepDeg >= 359.999) return { x: cx, y: cy };
    const rPad = sweepDeg < 28 ? 0.55 : sweepDeg < 60 ? 0.58 : 0.62;
    const r = outerR * rPad;
  ...
  const rotate = () =>
    setChores(prev => {
      const ids = ["tenley", "anna", "jake", "xiaowen"];
      const map = Object.fromEntries(ids.map((id, i) => [id, ids[(i + 1) % ids.length]]));
      return prev.map(c => {
        const next = members.find(m => m.id === map[c.assignee.id]) || c.assignee;
        return { ...c, assignee: next, color: next.color };
      });
    });
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5 flex flex-col items-center">
        <div className="relative w-64 h-64">
          <svg width="256" height="256" viewBox="0 0 256 256" className="rounded-full shadow-inner">
            <circle cx="128" cy="128" r="127" fill="#0b0b0f" />
            {slices.length === 1 && slices[0].sweep >= 359.999 ? (
              <circle cx="128" cy="128" r="120" fill={slices[0].m.color} />
            ) : (
              slices.map(s => (
                <path
                  key={s.m.id}
                  d={arcPath(128, 128, 120, s.start, s.end)}
                  fill={s.m.color}
            ...
            {(slices.length === 1 && slices[0].sweep >= 359.999
              ? [{ ...slices[0], labelAt: { x: 128, y: 128 } }]
              : slices.map(s => ({ ...s, labelAt: labelPoint(128, 128, 120, s.mid, s.sweep) }))
            ).map(s => {
              const p = s.labelAt;
              const fontSize = s.sweep < 22 ? 8 : s.sweep < 38 ? 9 : 10;
              return (
                <g key={`label-${s.m.id}`} transform={`translate(${p.x},${p.y})`}>
                  <rect x={-28} y={-11} rx={10} ry={10} width={56} height={22}
                        fill="rgba(0,0,0,0.45)" stroke="rgba(255,255,255,0.18)" strokeWidth="1" />
        ...
        <div className="flex gap-3 mt-4">
          <button onClick={() => setEditMode(v => !v)} className="px-3 py-1.5 text-xs rounded-lg bg-zinc-800 text-zinc-200">
            {editMode ? "Done" : "Customize"}
          </button>
          <button className="px-3 py-1.5 text-xs rounded-lg bg-indigo-600 text-white shadow" onClick={rotate}>
            Rotate
          </button>
        </div>
      </div>
          ...
          {chores.map(c => (
            <li key={c.id} className="p-4 flex items-center gap-3">
              <input
                ...
                checked={c.done}
                onChange={e =>
                  setChores(prev => prev.map(x => (x.id === c.id ? { ...x, done: e.target.checked } : x)))
                }
              ...
              <div className="flex-1 leading-tight">
                ...
                <p className="text-xs text-zinc-400">
                  Due in {c.dueInDays} day{c.dueInDays === 1 ? "" : "s"} • {c.recurrence}
                </p>
                {editMode && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {members.map(m => (
                      <button
                        ...
                        onClick={() =>
                          setChores(prev =>
                            prev.map(x => (x.id === c.id ? { ...x, assignee: m, color: m.color } : x))
                          )
                        }
              ...
              <Pill className="shrink-0">
                <NameWithDot name={c.assignee.name} color={c.assignee.color} />
              </Pill>
            </li>
          ))}
        </ul>
...
function PaymentsScreen({ members }) {
  const roommateCount = members.length;
...
function SummaryScreen({ members, chores }) {
  ...
  return (
    <div className="space-y-4">
      <TopBar title="Monthly summary" right={<Pill>{monthLabel}</Pill>} />
...
export default function RoomieFlowApp() {
  ...
  const [chores, setChores] = useState([
    { id: "trash",    title: "Take out trash",     dueInDays: 2, recurrence: "Weekly",    assignee: membersSeed[0], color: membersSeed[0].color, done: false },
    { id: "dishes",   title: "Dishes",             dueInDays: 1, recurrence: "Daily",     assignee: membersSeed[1], color: membersSeed[1].color, done: false },
    { id: "bathroom", title: "Clean bathroom",     dueInDays: 5, recurrence: "Bi-weekly", assignee: membersSeed[2], color: membersSeed[2].color, done: true  },
    { id: "living",   title: "Vacuum living room", dueInDays: 3, recurrence: "Weekly",    assignee: membersSeed[3], color: membersSeed[3].color, done: false },
  ]);
...
// repo cant be private without paying $$ -_-